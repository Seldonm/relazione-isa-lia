...
\end{lstlisting}
La variabile SPARK\_WORKER\_CORES, serve per poter configurare il numero di core da utilizzare sulla macchina (nel nostro caso settiamo il numero massimo di core che vogliamo utilizzare relativi ai nodi Slave).
La variabile SPARK\_WORKER\_MEMORY, consente di specificare quanta memoria dei rispettivi Workers si desidera allocare per l'esecuzione del software (ad esempio 1000m, 2g).
La variabile SPARK\_WORKER\_PORT / SPARK\_WORKER\_WEBUI\_PORT, consente di specificare la porta sulla quale avviare il nodo Master.
